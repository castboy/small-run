...
func main() {
    data := []byte("hello,i am chenkangxian,good night!")
    key := []byte("e3Rkp6XgcNqglGotAU6JlQ==")

    encryptData, err := AesECBEncrypt(data, key)
    if err != nil {
        log.Fatal(err)
    }

    pass64 := base64.StdEncoding.EncodeToString(encryptData)
    fmt.Printf("加密后:%v\n",pass64)
}